...
readonly FUNCTION_REPOSITORY_RELEASE_URL=https://github.com/dynatrace-oss/dynatrace-azure-log-forwarder/releases/download/release-0.1.6/
...
arguments:
    ...
    --arm-repository      Public repository url to ARM template
                            e.g. \"https://raw.githubusercontent.com/dynatrace-oss/dynatrace-azure-log-forwarder/master/dynatrace-azure-forwarder.json\"
...
if [[ "$ENABLE_USER_ASSIGNED_MANAGED_IDENTITY" == "true" ]]; then
  ...
  if [ -z "$SUBSCRIPTION_NAME" ]; then echo "No --subscription"; exit 1; fi
  if [ -z "$ARM_REPOSITORY" ]; then echo "No --arm-repository"; exit 1; fi
...
if [ "$ENABLE_USER_ASSIGNED_MANAGED_IDENTITY" = "true" ]; then
  ...
  --resource-group ${RESOURCE_GROUP} \
  --template-uri ${ARM_REPOSITORY}/${FUNCTION_ARM} \
...
az webapp deployment source config-zip --subscription ${SUBSCRIPTION_NAME} -n ${FUNCTIONAPP_NAME} -g ${RESOURCE_GROUP} --src ${FUNCTION_ZIP_PACKAGE}

if [[ "$ENABLE_USER_ASSIGNED_MANAGED_IDENTITY" == "true" ]]; then
  MANAGED_IDENTITY_RESOURCE_ID=$(az identity show --subscription ${SUBSCRIPTION_NAME} --name ${MANAGED_IDENTITY_RESOURCE_NAME} -g ${RESOURCE_GROUP} --query id --output tsv)
  az webapp identity assign --subscription ${SUBSCRIPTION_NAME} -n ${FUNCTIONAPP_NAME} -g ${RESOURCE_GROUP} --identities ${MANAGED_IDENTITY_RESOURCE_ID}
fi
